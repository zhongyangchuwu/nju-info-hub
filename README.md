...
Limited `fetch` and `ingest` select the newest requested source items after one-page recency lookahead, then attempt full-detail enrichment only for that fixed window. Unsupported public-WeChat/external items are kept as link-only observations without requesting their details; recognized campus-IP warning pages and NJU unified-identity redirects remain link-only after the ordinary public detail request. They do **not** continue into older items merely to reach a target number of full notices. Unrelated parse/fetch errors still fail. `discover` and `discover-pages` show public list metadata and acquisition classification without fetching details. Ingest persists an official-list observation immediately before each selected item's acquisition attempt. Ingest summaries count selected source items as `itemsObserved` and successfully persisted full notices as `noticesIngested`. Direct `fetch` never writes the database.
