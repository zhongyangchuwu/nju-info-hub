...
The static exporter reads the same persisted current revisions and writes `.json`, `.atom`, `.rss` for each selected source. With a validated public base URL it writes `catalog/sources.json`, describing only the sources in that publication and their absolute feed URLs, not the complete audited NJU source map or unpersisted channel/authority metadata. With a named source set it also writes versioned `catalog/sets.json`: source IDs from the resolved set and absolute links to its OPML (defaulting to `subscriptions/<setId>.opml`) and combined JSON/Atom/RSS bundle. The exporter owns and replaces the generated catalog directory; Pages copies static `index.html`, JavaScript, and CSS there only after export. The pilot selector is a client of those two JSON files, never a collector, SQLite API, or new backend. Arbitrary browser selections yield local OPML or copied individual feed URLs, not persistent combined feeds; only named, server-published sets have stable bundle URLs. The same resolved set generates OPML (independent per-source subscriptions) and combined feeds (one merged timeline) without duplicating notices or another crawl. Combined entries preserve stable item IDs and NJU URLs; Atom and RSS retain source identity through standard per-entry source elements. Pages uses one CS set over three public sources. The best-effort cache can be rebuilt after eviction; static readers do not collect. Per-source feeds, `catalog/sources.json`, CS OPML, and CS bundles are publicly live from run #35989422809; `catalog/sets.json` and the static selector are also live after run #35991982111.
