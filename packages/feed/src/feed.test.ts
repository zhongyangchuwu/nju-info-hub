...
describe("JSON Feed builder", () => {
  it("preserves full-entry identity, source provenance, day precision, body and every attachment", () => {
    ...
    const laterObservation = buildJsonFeed(source, [{ ...notice, provenance: {
      ...notice.provenance,
      fetchedAt: "2026-09-24T11:00:00.000Z",
    } }]).items[0]!;
    expect(laterObservation).not.toHaveProperty("date_published");
    expect(laterObservation._nju).toMatchObject({
